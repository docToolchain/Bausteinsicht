...
For a detailed walkthrough, see the link:../tutorial/01_getting_started.html[Getting Started Tutorial] or the https://paul-fleischmann.com/en/projekte/bausteinsicht/tutorial/[online tutorial].
...
1. **A virtual X display** — provided by `xvfb`.
The `drawio-export` wrapper script handles this automatically.

2. **A running D-Bus system daemon** — Electron requires D-Bus for IPC.
Without it, export fails with `"Export failed"` or `"input file/directory not found"`.
+
[source,bash]
----
# Start dbus (required once per container session)
sudo mkdir -p /run/dbus
sudo dbus-daemon --system --fork
----

3. **The `--no-sandbox` flag** — required in containers without user namespace support.
The `drawio-export` wrapper includes this flag.
...
NOTE: GPU errors like `"Exiting GPU process due to errors during initialization"` in stderr are harmless — draw.io falls back to software rendering.
...
* link:../tutorial/01_getting_started.html[Getting Started Tutorial] — step-by-step walkthrough
