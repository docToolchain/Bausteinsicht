...
| BR-014 | A view's `layout`, if set, must be one of `layered`, `grid`, `none`, `nested`. | error | `validate.go` `validateViews`
| BR-014a | A view with `layout: "nested"` must declare a `scope` (nested renders the scope's container nesting as concentric containers). | error | `validate.go` `validateViews`
