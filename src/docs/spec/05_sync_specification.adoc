...
* `nested` — renders the model's container nesting as *concentric containers*: each container's children are placed inside it and every container is sized bottom-up from its content, so a nested model (`ordering ⊃ adapters ⊃ ports ⊃ domain`) becomes concentric hexagon rings without manual arrangement. Elements' `metadata.side` (`top`/`bottom`) drives vertical placement within a ring (driving/inbound on top, driven/outbound at the bottom); elements outside the scope container are placed around it. After placement, `computeNestedLayout` re-parents each element to its immediate container and sets the container sizes (`layoutResult.ParentOf` / `ContainerSizes`), so the draw.io nesting mirrors the model. Used by the `examples/hexagonal-ddd` Ports & Adapters view. A `nested` view must declare a `scope` — validation (BR-014a) rejects a scope-less `nested` view rather than silently falling back to another mode.
+
NOTE: `nested` guarantees correct containment only on a *fresh* page — i.e. the first sync of a view, or after `sync --relayout`. Elements added incrementally to an existing nested page take the cursor-based placement path (`computePlacement`) like every other layout, so they are parented flat under the scope boundary and the containers are not resized until the next full relayout. Run `sync --relayout` (or delete the page) to restore the rings after such edits.
