...
* `layered` (the default, also used when `layout` is empty) — elements are arranged in horizontal rows grouped by kind, in the specification's element order. A view's scoped children are additionally grouped into BFS rows by relationship distance from actors (row 1 = elements connected to an actor, row 2 = elements connected to row 1, and so on), and *within* each row, elements are ordered to minimize edge crossings with the rows immediately above and below (`internal/sync/layout.go:minimizeCrossings` — a barycenter-method sweep from Sugiyama-style layered graph drawing, #607). This crossing minimization only has rows to work with when the BFS seed exists: views with no actor in scope (e.g. a container-level view scoped to internal packages only) collapse to a single unordered row, alphabetically sorted. All other elements (outside the scoped-children BFS) are ordered alphabetically.
