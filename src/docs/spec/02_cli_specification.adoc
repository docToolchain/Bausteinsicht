...
**Constraint rule types:** each entry in the model's `constraints` array selects one `rule` and supplies its parameters. All rules are kind-based and evaluate over the flattened element set and the `relationships` list.
