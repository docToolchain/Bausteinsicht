...
* Creates the output directory if it does not exist.
* Before rendering each view, prints a `WARNING: view "<key>" resolves to 0 elements — check its scope/include/exclude; the exported diagram will be empty` to stderr if the view's resolved element set is empty. This does not fail the export (exit code stays `0`) — it only surfaces a diagram that would otherwise silently look successful despite being empty (#512).
...
**Constraint rule types:** each entry in the model's `constraints` array selects one `rule` and supplies its parameters. Rules evaluate over the flattened element set and the `relationships` list. Endpoint/element selectors are flexible — beyond the `*-kind` forms shown below, they also accept an exact ID (`from`/`to`/`element`), a tag (`from-tag`/…), or an any-of allow-list (`from-tags`/`from-kinds`/…), matched in that order of specificity (#542).
...
| `no-relationship`
| `from-kind`, `to-kind`
| No element of `from-kind` may relate to any element of `to-kind`.

| `allowed-relationship`
| `to-kind`, `from-kinds`
| Only elements whose kind is in `from-kinds` may point at `to-kind`.

| `no-outgoing-dependency`
| `from-kind`
| No element of `from-kind` may be the source of any relationship. Use it for pure-interface roles such as a hexagonal port, which is implemented and used by others but must depend on nothing itself.

| `required-field`
| `element-kind`, `field`
| Every element of `element-kind` must set `field` (`title`, `description`, or `technology`).

| `max-depth`
| `max`
| No element may be nested deeper than `max` levels (root elements are depth 1).

| `no-circular-dependency`
| (none)
| The relationship graph must be acyclic.

| `technology-allowed`
| `element-kind`, `technologies`
| Elements of `element-kind` that declare a technology may only use one from `technologies` (unset technology is not flagged).
|===
