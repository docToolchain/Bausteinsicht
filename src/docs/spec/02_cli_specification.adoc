...
| `--title`
| View title (display name) (required).
| Yes
| --
...
* Checks for duplicate view keys and rejects if already exists.
* Creates a new view in the JSONC file using comment-preserving insertion.

**Exit codes:**

* `0` -- view added successfully
* `1` -- scope or include element not found, view key already exists, or missing required flag
* `2` -- model file not found or I/O error
