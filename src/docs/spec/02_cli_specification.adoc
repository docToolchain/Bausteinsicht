...
NOTE: SVG export always passes draw.io's `--embed-svg-fonts false` internally (not a flag; not applicable to PNG). Without it, draw.io falls back to rasterizing each text label as an embedded base64 PNG whenever it can't embed a font subset headlessly, inflating file size by roughly 20× for no visual benefit (#560). Text then relies on the viewer's system fonts — the normal trade-off for unembedded SVG text, and how GitHub's own AsciiDoc blob preview renders these diagrams.
