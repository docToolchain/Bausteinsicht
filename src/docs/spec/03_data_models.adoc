...
| string[]
| no
| Element IDs to include (supports `*` wildcard for children)

| `exclude`
| string[]
| no
| Element IDs to exclude from the view
...
Each view is a separate `<diagram>` element. The `id` attribute is the view key from the model.
