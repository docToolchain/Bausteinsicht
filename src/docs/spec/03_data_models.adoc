...
| string
| no
| Auto-layout mode for new pages: `"layered"` (default), `"grid"`, `"none"`, or `"nested"`. Layered groups elements by kind tier; grid places alphabetically in a grid; none uses a horizontal row; nested renders the model's container nesting as concentric containers, sized bottom-up (used for hexagonal Ports & Adapters rings).
|===
...
type View struct {
    ...
    Description string   `json:"description,omitempty"`
    Layout      string   `json:"layout,omitempty"` // layered (default)|grid|none|nested
}
