...
| 2026-06-27 (XMI importer)
| Added `internal/importer/xmi/` (issue #401). New XMI attack surface reviewed: (1) XXE — mitigated by `hasDOCTYPE()` pre-check and `dec.Entity = map[string]string{}` to disable entity expansion; (2) deep hierarchy DoS — mitigated by `maxDepth=50` cap in `parseElem`, same pattern as SEC-007 (AUTOSAR exports can reach depth 23; 50 gives headroom); (3) ID injection via crafted element names — sanitized by `sanitizeID()` (same function used for draw.io, same SEC-013 fix applied). XMI version is detected from root element; non-2.1 versions emit a warning. No new SEC findings introduced.
