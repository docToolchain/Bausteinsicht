...
The non-element `add` subcommands — `add relationship`, `add view`, `add specification`, and `add-from-pattern` — cover relationships between existing elements, views, specification entries, and pattern-based bulk generation. `add-from-pattern list` (equivalently reachable as `add pattern list`) lists the patterns available to apply. All are pure model edits, same shape as `add element` (see LLM-Driven Modification above).
