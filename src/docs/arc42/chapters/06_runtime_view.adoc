...
The non-element `add` subcommands — `add relationship`, `add view`, `add specification`, and `add-from-pattern` — cover relationships between existing elements, views, specification entries, and pattern-based bulk generation. All are pure model edits, same shape as `add element` (see LLM-Driven Modification above).
...
Cross-cutting setup/maintenance commands, not part of the daily edit-sync loop: multi-model workspaces, auto-layout, template scaffolding, and JSON Schema regeneration for IDE support.
...
Two `interactive`-group commands outside the REPL itself: metric heatmap overlays on diagrams, and ADR (Architecture Decision Record) scaffolding.
