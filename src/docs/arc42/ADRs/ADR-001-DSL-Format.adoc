...
The established DSL from the Structurizr ecosystem (Simon Brown / C4 model).
Used by Structurizr, Structurizr Lite, and various community tools.
...
A newer C4-inspired DSL with a modern developer experience.
Uses Langium for parsing, ships with a VS Code extension and a web-based viewer.
...
Architecture modeling using PlantUML with the C4-PlantUML extension library.
Leverages the widely-used PlantUML rendering engine.
...
| Learnability (×5)                | +5  | -5  | 0 | 0  | 0  | +5
| IDE support (×5)                 | +5  | +5  | 0 | -5 | 0  | +5
| LLM friendliness (×4)           | +4  | +4  | 0 | 0  | -4 | +4
| Parser / tooling effort (×4)    | +4  | +4  | 0 | 0  | 0  | -4
| Bidirectional sync (×5)         | +5  | -5  | 0 | -5 | -5 | -5
| Syntax compactness (×3)         | -3  | -3  | 0 | 0  | 0  | -3
| Portability (×2)                | +2  | -2  | 0 | +2 | -2 | +2
| Expressiveness (×2)             | -2  | +2  | 0 | -2 | +2 | -2
| *Total*                       | *+20* | *0* | *0* | *-10* | *-9* | *+2*
|===
